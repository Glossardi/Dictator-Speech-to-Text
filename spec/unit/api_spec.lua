...
describe("api module", function()
    ...
    describe("validateApiKey", function()
        ...
        it("should accept API key without 'sk-' prefix (for non-OpenAI providers)", function()
            local valid, err = api.validateApiKey("long-enough-api-key-for-other-providers")
            assert.is_true(valid)
            assert.is_nil(err)
        end)
        ...
        it("should reject API key that is too short", function()
            ...
            assert.is_false(valid)
            assert.are.equal("API key too short (minimum 20 characters)", err)
        end)
