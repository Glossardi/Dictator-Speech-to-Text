...
describe("rate_limiter module", function()
    ...
    describe("initialization", function()
        it("should initialize with default configuration", function()
            rate_limiter.init()
            assert.are.equal(3, rate_limiter.maxTokens)
            assert.are.equal(3, rate_limiter.tokens)
            assert.are.equal(0.05, rate_limiter.refillRate)  -- 3/60 = 0.05
        end)
    ...
    describe("token refilling", function()
        before_each(function()
            rate_limiter.init()
            -- Start with full bucket (3 tokens)
            assert.are.equal(3, rate_limiter.tokens)
        end)
        ...
        it("should refill tokens over time", function()
            ...
            mock_time = mock_time + 20  -- 20 seconds later
            rate_limiter.refillTokens()
            
            -- 20 seconds * 0.05 tokens/sec = 1 token
            assert.are.equal(1, rate_limiter.tokens)
        end)
        
        it("should not exceed max tokens", function()
            rate_limiter.tokens = 2
            mock_time = mock_time + 100  -- 100 seconds later (would add 5 tokens)
            rate_limiter.refillTokens()
            
            assert.are.equal(3, rate_limiter.tokens)  -- Capped at maxTokens
        end)
    ...
    describe("canMakeRequest", function()
        ...
        it("should account for time passing", function()
            ...
            mock_time = mock_time + 20  -- Add 1 token
            assert.is_true(rate_limiter.canMakeRequest())
        end)
    end)
    ...
    describe("consumeToken", function()
        ...
        it("should allow multiple requests within limit", function()
            ...
            local allowed3, _ = rate_limiter.consumeToken()
        ...
        it("should calculate correct wait time", function()
            ...
            assert.is_false(allowed)
            -- 1 token needed / 0.05 tokens per second = 20 seconds
            assert.are.equal(20, waitTime)
        end)
    ...
    describe("getStatus", function()
        ...
        it("should return current status", function()
            ...
            assert.are.equal(3, status.tokens)
            assert.are.equal(3, status.maxTokens)
            assert.are.equal(0.05, status.refillRate)
        ...
        it("should reflect consumed tokens", function()
            ...
            assert.are.equal(2, status.tokens)
            assert.is_true(math.abs(status.percentAvailable - 66.67) < 0.1)
        end)
    ...
    describe("token bucket algorithm", function()
        ...
        it("should handle burst requests followed by refill", function()
            ...
            -- Wait half the window (30s = 1.5 tokens)
            ...
            -- But not two
            allowed, _ = rate_limiter.consumeToken()
            assert.is_false(allowed)
        end)
    end)
end)
